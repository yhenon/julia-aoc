...
mutable struct OutputConjunction
    inputs::Dict{String, String}
...
function parse()
    ...
    open("inputs/day20.txt") do f
        for l in eachline(f)
            ...
            elseif source[1] == '&'
                if name == "dd"
                    d[name] = OutputConjunction(Dict(), Dict(), dests, repeat(["low"], length(dests)), name)
...
function propagate(q::Queue{Tuple{String, String, String}}, source::String, message::String, mymodule::OutputConjunction)
    mymodule.inputs[source] = message
    all_high = false
    if message == "high"
        if source ∉ keys(mymodule.cycles)
            mymodule.cycles[source] = cycle_num
        end
    end
    if length(mymodule.cycles) == length(mymodule.inputs)
        println(lcm(values(mymodule.cycles)...))
        exit()
    end
    if "low" ∉ values(mymodule.inputs)
        all_high = true
